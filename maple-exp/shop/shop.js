...
const resetArr = () => {
  for (let i = 0; i < data.MPOINT + 1; i++) arr[i][0] = arr[i][1] = arr[i][1] = 0;
}
/**
 *
  ...
  for (let i = 0; i < 5; i++) {
    if (n + data.elixirCoin[i] <= POINT) {
      ...
      if (lv_compare(data.user, arr[n + data.elixirCoin[i]])) {
        ...
        if(lv_compare(arr[s], arr[ret])) ret = s;
      }
    } else break;
  }
  return ret;
...
const main = (lv, xp, po) => {
  data.user.level = lv;
  data.user.percent = xp;//Here will get data.user level, exp and point.
  data.setByPercent();
  const POINT = po;
